...
// =============================
exports.runVirtualTryOn = onDocumentCreated(
  {
  ...
  async (event) => {
    ...
    const tryOnId = event.params.tryOnId;
    ...
    try {
      ...
      const resultImageUrl = prediction.output?.[0] || null;
...
// =============================
exports.analyzeClothing = onObjectFinalized(async (event) => {
  ...
  try {
    ...
    const labels = labelsResult.labelAnnotations.map(
      (l) => l.description
    );
...
exports.generateOutfit = onRequest(async (req, res) => {
  try {
